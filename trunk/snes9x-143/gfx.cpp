...
static void S9xDisplayString (const char *string, int lines, bool linesFromBottom, int pixelsFromLeft, bool allowWrap);
...
void S9xSetupOBJ ()
{
	...
	if(!PPU.OAMPriorityRotation || !(PPU.OAMFlip&PPU.OAMAddr&1)){
		...
		do {
			...
			int HPos=PPU.OBJ[S].HPos; if(HPos==-256) HPos=256/*0*/;
			if(HPos>-GFX.OBJWidths[S] && HPos<=256)
			{
				if(HPos<0){
					GFX.OBJVisibleTiles[S]=(GFX.OBJWidths[S]+HPos+7)>>3;
				} else if(HPos+GFX.OBJWidths[S]>=257/*>255*/){
					GFX.OBJVisibleTiles[S]=(257/*256*/-HPos+7)>>3;
				} else {
		...
		for(int Y=0; Y<SNES_HEIGHT_EXTENDED; Y++){
			GFX.OBJLines[Y].RTOFlags=Y?0:GFX.OBJLines[Y-1].RTOFlags/*GFX.OBJLines[Y-1].RTOFlags:0*/;
...
if(Settings.BGLayering) {
			...
			{
				SelectTileRenderer (!GFX.PseudoHires && PPU.OBJ [S].Palette < 4);
			}
...
void S9xUpdateScreen () {
    ...
    // This change may cause a crash with old save-state
    GFX.PseudoHires = (Memory.FillRAM [0x2133] & 8);

#elif defined(JP_FIX)

    GFX.PseudoHires = (Memory.FillRAM [0x2133] & 8) != 0 &&
...
static void S9xDisplayFrameRate ()
{
	...
	DisplayString(string, 1, true, 1+display_width-display_fontheight*len, false);
}
...
/*
	...
	int enabledCount = 0;
	{
		for(int J = 0; J < 5; J++)
		{
			if((IPPU.Joypads[J] & 0xffff) != 0)
				activeCount++;

			if(S9xMovieActive() && S9xMovieControllers() & (1<<J)
			|| !S9xMovieActive() && Joypad[J].Enabled)
				enabledCount++;
		}
	}
	bool singlePlayer = (enabledCount <= 1);
*/
...
//			|| !S9xMovieActive())
	...
	for(int id = 0; id < 5; id++)
	{
		...
		if(!(IPPU.Joypads[id] & 0xffff) && id > 0)
			skip = true;
		else
		{
			...
			for (int i=0; i < 15; i++)
			{
				int j = KeyOrder[i];
				int mask = (1 << (j+1));
				string[strlen("P?: ")+i]= ((IPPU.Joypads[id] & mask)!=0) ? KeyMap[j] : ' ';
			}
		}
		...
		DisplayString(string, line, true, 1, false);
...
void (*S9xCustomDisplayString) (const char *string, int lines, bool linesFromBottom, int pixelsFromLeft, bool allowWrap) = NULL;
