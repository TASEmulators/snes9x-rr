...
// a way of selectively replacing certain inputs with a programmatic source,
// made with peripherals and movie recording and input display in mind.
// this is mainly used to implement the Lua function joypad.set()
// this struct is only the internal implementation.
// the "public" interface is below (e.g. AddCommandTransformAxis)
// currently it ONLY supports non-joypad peripherals, because joypads had another solution already (lua_joypads) that I was too lazy to rip out
static struct CommandConversions
{
	uint32 typeFilter;

	static const int MAX_FILTERS = 64;
	struct Conversion
	{
		uint32 type;
		uint32 filterCheck;
		...
		int16 data2;
		int16 data2Mask;
	} conversions [MAX_FILTERS];
	int numConversions;

	void AddConversion(Conversion cf)
	{
		for(int i = 0; i < numConversions; i++)
		{
			Conversion& c = conversions[i];
			if(c.type == cf.type && c.filterMask == cf.filterMask && c.filterCheck == cf.filterCheck)
			{
				// if the filter matches, replace or augment the existing conversion
				c.data1 = (cf.data1 & cf.data1Mask) | (c.data1 & ~cf.data1Mask);
				c.data2 = (cf.data2 & cf.data2Mask) | (c.data2 & ~cf.data2Mask);
				c.data1Mask |= cf.data1Mask;
				c.data2Mask |= cf.data2Mask;
				return;
			}
		}
		assert(numConversions < MAX_FILTERS);
		if(numConversions >= MAX_FILTERS)
			return;
		conversions[numConversions] = cf;
		numConversions++;
		typeFilter |= (1 << cf.type) & 0xFFFFFFFF;
	}
	...
	void TransformCommand(s9xcommand_t& cmd, int16& data1, int16& data2)
	{
		if(!(typeFilter & ((1 << cmd.type) & 0xFFFFFFFF)))
			return;
		for(int i = 0; i < numConversions; i++)
		{
			Conversion& c = conversions[i];
			if(cmd.type == c.type && (cmd.button.multi_idx & c.filterMask) == c.filterCheck)
			{
				// convert the data
				data1 = (c.data1 & c.data1Mask) | (data1 & ~c.data1Mask);
				data2 = (c.data2 & c.data2Mask) | (data2 & ~c.data2Mask);
			}
		}
	}
} s_luaCommandConversions = {0};

void AddCommandTransformAxis(controllers type, int idx, int16 val, bool8 axis)
{
	s9xcommand_t c={0};
	switch(type) {
		case CTL_MOUSE: idx ? (c.pointer.aim_mouse1=1) : (c.pointer.aim_mouse0=1); break;
		case CTL_SUPERSCOPE: c.pointer.aim_scope=1; break;
		case CTL_JUSTIFIER: idx ? (c.pointer.aim_justifier1=1) : (c.pointer.aim_justifier0=1); break;
		default: return;
	}
	CommandConversions::Conversion cf = {S9xPointer, c.button.multi_idx, c.button.multi_idx, val, ~-axis, val, -axis};
	s_luaCommandConversions.AddConversion(cf);
}
void AddCommandTransformButton(controllers type, int idx, bool8 on, uint8 mask)
{
	...
	{
		case CTL_MOUSE: {
			c.button.mouse.left=!!(mask&SNESMOUSE_LEFT);
			c.button.mouse.right=!!(mask&SNESMOUSE_RIGHT);
			m = c;
			c.button.mouse.idx=idx;
			m.button.mouse.idx=1;
			cmdType = S9xButtonMouse;
		} break;
		case CTL_SUPERSCOPE: {
			c.button.scope.fire=!!(mask&SUPERSCOPE_FIRE);
			c.button.scope.cursor=!!(mask&SUPERSCOPE_CURSOR);
			c.button.scope.turbo=!!(mask&SUPERSCOPE_TURBO);
			c.button.scope.pause=!!(mask&SUPERSCOPE_PAUSE);
			c.button.scope.aim_offscreen=!!(mask&SUPERSCOPE_OFFSCREEN);
			m = c;
			cmdType = S9xButtonSuperscope;
		} break;
		case CTL_JUSTIFIER: {
			mask<<=idx;
			c.button.justifier.trigger=!!(mask&JUSTIFIER_TRIGGER);
			c.button.justifier.start=!!(mask&JUSTIFIER_START);
			//c.button.justifier.select=!!(mask&JUSTIFIER_SELECT);
			c.button.justifier.aim_offscreen=!!(mask&0x02);
			m = c;
			c.button.justifier.idx=idx;
			m.button.justifier.idx=1;
			cmdType = S9xButtonJustifier;
		} break;
		default: return;
	}
	CommandConversions::Conversion cf = {cmdType, c.button.multi_idx, m.button.multi_idx, on,~0, 0,0};
	s_luaCommandConversions.AddConversion(cf);
}
void ClearCommandTransforms()
{
	s_luaCommandConversions.ClearConversions();
}

void S9xApplyCommand(s9xcommand_t cmd, int16 data1, int16 data2){
    int i;

	if(s_luaCommandConversions.numConversions)
		s_luaCommandConversions.TransformCommand(cmd, data1, data2);
    ...
    switch(cmd.type){
      ...
      case S9xButtonJustifier:
        ...
        //if(cmd.button.justifier.select) i|=JUSTIFIER_SELECT;
...
#if defined(NETPLAY_SUPPORT) && !defined(WIN32)
