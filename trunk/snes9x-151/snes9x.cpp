...
void S9xUsage ()
{
    ...
    S9xMessage(S9X_INFO, S9X_USAGE, "-ignoreframes <num>             Start capturing at a specified frame number.");
    ...
    S9xMessage(S9X_INFO, S9X_USAGE, "-loadlua <filename>             Load a lua script");
    S9xMessage(S9X_INFO, S9X_USAGE, "");
    S9xExtraUsage();
...
char *S9xParseArgs (char **argv, int argc)
{
	...
	if (*argv[i] == '-')
	{
            ...
            else if (strcasecmp(argv[i],"-ahiunit")==0)
            {
             if (i+1<argc)
             {
              ...
              unit=atoi(argv[++i]);
                ...
                dumpstreams = 1;
            else if (strcasecmp (argv[i], "-ignoreframes") == 0)
                ignoreframes = atoi(argv[++i]);
		...
		if (!argv[i])
            ...
            else if (strcasecmp(argv[i], "-loadlua") == 0) {
                i++;
                if (!argv[i])
                    abort();
                strcpy(luascript, argv[i]);
            }
...
void S9xLoadConfigFiles(char **argv, int argc){
    ...
    if(conf.Exists("Sound::AltDecode")){
        Settings.AltSampleDecode=(uint8)conf.GetInt("Sound::AltDecode", 1);
    }
