...
void S9xUpdateFrameCounter (int offset)
{
	...
	if(!Settings.DisplayFrame)
		*GFX.FrameDisplayString = 0;
		...
		if (Settings.CounterInFrames)
			...
			FrameCountToTime(GFX.FrameDisplayString, max(0,(int)(Timings.TotalEmulatedFrames+offset)), Memory.ROMFramesPerSecond);
		if (!Settings.OldFashionedFrameCounter)
	...
	if (!Settings.DisplayLagCounter)
		*GFX.LagCounterString = 0;
		...
		if (Settings.CounterInFrames)
			sprintf(GFX.LagCounterString, "%d", max(0,(int)(Timings.LagCounter)));
		else
			FrameCountToTime(GFX.LagCounterString, max(0,(int)(Timings.LagCounter)), Memory.ROMFramesPerSecond);
		//if (!pad_read)
