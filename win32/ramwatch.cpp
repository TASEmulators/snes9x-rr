...
bool Save_Watches()
{
	const char* slash = std::max(strrchr(gamefilename, '|'), std::max(strrchr(gamefilename, '\\'), strrchr(gamefilename, '/')));
			...
			switch(LOWORD(wParam))
			{
				...
				case IDC_C_ADDCHEAT:
				{
					...
					if(watchIndex >= 0)
					{
						...
						extern int CALLBACK DlgCheatSearchAdd(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam);
