...
INLINE void S9xSetWordWrapped (uint16 Word, uint32 Address, enum s9xwrap_t w, enum s9xwriteorder_t o, bool free)
{
    uint32 mask=MEMMAP_MASK&(w==WRAP_PAGE?0xff:(w==WRAP_BANK?0xffff:0xffffff));
    if ((Address & mask) == mask)
    {
        if(!o) S9xSetByteWrapped(Word&0x00FF, Address, free);
        switch(w){
          case WRAP_PAGE:
            {
                PC_t a;
                a.xPBPC = Address;
                a.B.xPCl++;
                S9xSetByteWrapped(Word>>8, a.xPBPC, free);
            }
          case WRAP_BANK:
            {
                PC_t a;
