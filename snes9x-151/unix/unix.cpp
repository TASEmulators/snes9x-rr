...
void S9xParseArg (char **argv, int &i, int argc)
{
    ...
    if (i + 1 < argc) {
        ...
        sound_device = argv[++i];
#else
        S9xMessage(S9X_WARNING, S9X_USAGE, "Sound support is not available!");
#endif
        } else S9xUsage ();
...
//static void NSRTControllerSetup()
void S9xPostRomInit()
{
...
int main (int argc, char **argv)
{
    ...
    Settings.SoundPlaybackRate = 4; // 4 -> 32kHz
	...
	if (luascript[0] != 0) // if the first byte's 0, it's an empty string
	  S9xLoadLuaCode (luascript);
    }
    else
    {
	S9xReset ();
    ...
    if (Settings.NetPlay)
    {
	NetPlay.MaxFrameSkip=10; // FF to Nach: How can I make this configurable?

	if (!S9xNPConnectToServer(Settings.ServerName, Settings.Port,
					Memory.ROMName))
	{
	    fprintf (stderr, "Failed to connect to Snes9x netplay"
                     " server \"%s\" on port %d.\n",
	...
	if (NP_Activated)
	{
		...
		{
			fprintf(stderr, "Lost netplay connection to server\n");
			S9xExit();
		}
	}
#endif
	...
	if (NP_Activated)
	{
		...
		/* Now restore the joypad input so that ProcessEvents works as expected */
		for (int J = 0; J < 8; J++)
...
char* osd_GetPackDir()
{
  ...
  if(strlen(S9xGetDirectory(SNAPSHOT_DIR))!=0)
...
{
    S9xLuaGui((uint16*)GFX.Screen, Width, Height, 32, GFX.Pitch/2); // bpp variable?
...
void S9xSyncSpeed ()
{
    ...
    if (dumpstreams && !Settings.HighSpeedSeek)
      return;
#ifdef NETPLAY_SUPPORT
    ...
    {
      // Unforeseen events, such as SIGSTOP, severe system lag, etc
...
bool8 S9xOpenSoundDevice (int mode, bool8 stereo, int buffer_size)
{
    ...
    printf ("Sound Device: %s, Rate: %d, Buffer size: %d, 16-bit: %s, Stereo: %s, Encoded: %s\n",
	    sound_device,
	    so.playback_rate, so.buffer_size, so.sixteen_bit ? "yes" : "no",
	    so.stereo ? "yes" : "no", so.encoded ? "yes" : "no");
