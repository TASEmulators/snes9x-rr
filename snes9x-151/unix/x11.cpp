...
static void Repaint(bool isFrameBoundry = false);
...
void S9xPutImage (int snes_width, int snes_height)
{
     if(ignoreframes > 0)
     {
         --ignoreframes;
         if(!(ignoreframes%100))
             fprintf(stderr,"\rignoreframes=%-10d%s", ignoreframes, Settings.TurboMode?"[TURBO]":"       ");
         return;
     }
#ifdef USE_GLIDE
...
static void Repaint(bool isFrameBoundry)
{
    ...
    if (isFrameBoundry)
    {
       ...
       // Log
       VideoLogger(GUI.image_date, GUI.last_snes_width, GUI.last_snes_height, GUI.bytes_per_pixel, GUI.bytes_per_line);
