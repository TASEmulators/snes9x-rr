...
void S9xDoHBlankProcessing ()
{
	...
	if (CPU.V_Counter == PPU.ScreenHeight + 3)
	    S9xUpdateJoypadMemory();

	// separated from S9xUpdateJoypadMemory because the +3 was causing keypresses to be displayed 1 frame behind,
	// but just removing the +3 always would cause some games (Pocky & Rocky) to fail to get input.
	// (with this change, games emulate identically to before, only the keypress GUI display timing is better)
	if (CPU.V_Counter == PPU.ScreenHeight)
