...
void JoystickChanged( short ID, short Movement)
{
	...
	JoyKey |= (WORD)(ID << 8);
	JoyKey |= Movement;
	SendMessage(funky,WM_APP+45,JoyKey,0);
//	KillTimer(funky,tid);
}
...
static LRESULT CALLBACK InputCustomWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		InvalidateRect(icp->hwnd, NULL, FALSE);
		UpdateWindow(icp->hwnd);
		SendMessage(pappy,WM_APP+43,wParam,(LPARAM)hwnd);
...
//		tid = wParam;
	...
	{
		selectedItem = NULL;
		SendMessage(pappy,WM_APP+46,wParam,(LPARAM)hwnd); // refresh fields on deselect
		break;
	}
