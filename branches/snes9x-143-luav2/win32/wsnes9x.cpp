...
SJoypad TurboToggleJoypadStorage [5] = {
	{ 
			false,                                  
				0, 0, 0, 0,
				0, 0, 0, 0,
...
void HotkeyNewLuaScript(bool justPressed);
void HotkeyCloseLuaScripts(bool justPressed);
void HotkeyMostRecentLuaScript(bool justPressed);
...
void InitCustomKeys (SCustomKeys *keys)
{
	...
	keys->NewLuaScript.handleKeyDown = HotkeyNewLuaScript;
	keys->CloseLuaScripts.handleKeyDown = HotkeyCloseLuaScripts;
	keys->MostRecentLuaScript.handleKeyDown = HotkeyMostRecentLuaScript;
	...
	keys->NewLuaScript.name = _T("New Lua Script");
	keys->CloseLuaScripts.name = _T("Close Lua Scripts");
	keys->MostRecentLuaScript.name = _T("Most Recent Lua Script");
...
//	4, 4, 8, 12, 17, 20, 35, 70, 140, 300, 500, 1000, 1000
//};
...
{
	static char string [100];
//			sprintf (InfoString, "Speed: %.0f%% (%d ms/frame)", ((Settings.PAL?20.0f:17.0f) * 100.0f) / (float)Settings.FrameTime, Settings.FrameTime);
...
//	}
	...
	{
//		if (Settings.UpAndDown)
//			syncFlags |= MOVIE_SYNC_LEFTRIGHT;
//		else
//			syncFlags &= ~MOVIE_SYNC_LEFTRIGHT;
						 ...
						 LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_FILE_MOVIE_RECORD:
			{
					...
					op.Path[0]!='\0')
				{
					if(!Settings.ShutdownMaster)
					{
						...
						{
							seenItOnce = true;
							MessageBox(hWnd, MOVIE_SHUTDOWNMASTER_WARNING_V14, SNES9X_WARN, MB_OK);
						}
					}
		...
		case ID_OPTIONS_DISPLAY:
			{
				RenderFilter old_scale = GUI.NextScale;
				...
				case IDM_7110_RTC:
					{
						...
						case ID_VIDEO_TRANSPARENCY:
							...
							S9xDisplayStateChange (WINPROC_TRANSPARENCY, Settings.Transparency);
							break;
...
void HotkeyNewLuaScript (bool justPressed) { PostMenuCommand(IDC_NEW_LUA_SCRIPT); }
void HotkeyCloseLuaScripts (bool justPressed) { PostMenuCommand(IDC_CLOSE_LUA_SCRIPTS); }
void HotkeyMostRecentLuaScript (bool justPressed) {} // TODO: HotkeyMostRecentLuaScript
...
static void CheckMenuStates ()
{
	...
	mii.fState = Settings.Mode7Interpolate ? MFS_CHECKED : MFS_UNCHECKED;
	SetMenuItemInfo (GUI.hMenu, ID_VIDEO_BILINEARMODE7, FALSE, &mii);
			...
			case IDOK:
				{
					...
					set->UseWIPAPUTiming=IsDlgButtonChecked(hDlg, IDC_WIP1);
...
INT_PTR CALLBACK DlgEmulatorProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		if(DirectDraw.Clipped) S9xReRefresh();
		{
			...
			SetDlgItemText(hDlg, IDC_CONFIG_NAME_BOX, using_conf_not_cfg ? "snes9x14.conf" : "snes9x14.cfg");
	...
	case WM_COMMAND:
		{
			...
			case IDOK:
				GetDlgItemText(hDlg, IDC_FRAME_NUMBER, frame_number_text, MAX_PATH);
                                ...
                                {
					Settings.HighSpeedSeek = frame_dest - S9xMovieGetFrameCounter() + 1;
					Settings.Paused = false;
					Settings.FrameAdvance = false;
					GUI.FrameAdvanceJustPressed = 0;
                                }
	...
	case WM_COMMAND:
		...
		case IDOK:
			if(IsDlgButtonChecked(hDlg, IDC_AUTOFRAME))
			{
				...
				Settings.AutoMaxSkipFrames=(uint32)_ttoi(temp) + 1;
			}
			else
			{
				GetDlgItemText(hDlg,IDC_SKIPCOUNT,temp,80);
				Settings.SkipFrames=(uint32)_ttoi(temp);
			}
...
INT_PTR CALLBACK DlgFunky(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	static bool prevStretch, prevAspectRatio, prevMode7Interpolate, prevHeightExtend, prevMessagesInImage, prevVideoMemory;
...
//		SendDlgItemMessage(hDlg,IDC_FILTERBOX2,CB_SETCURSEL,(WPARAM)GUI.NextScaleHiRes,0);
	...
	case WM_COMMAND:
		...
		case IDC_BILINEARMD7:
			if(!in_display_dlg) break;
			Settings.Mode7Interpolate = (bool)(IsDlgButtonChecked(hDlg,IDC_BILINEARMD7)==BST_CHECKED);
			// refresh screen, so the user can see the new mode
			if(GUI.outputMethod==DIRECT3D)
				Direct3D.changeRenderSize(0,0);
...
{
	SendDlgItemMessage(hDlg,IDC_UP,WM_APP+44,Joypad[index].Up,0);
	SendDlgItemMessage(hDlg,IDC_LEFT,WM_APP+44,Joypad[index].Left,0);
	SendDlgItemMessage(hDlg,IDC_DOWN,WM_APP+44,Joypad[index].Down,0);
	SendDlgItemMessage(hDlg,IDC_RIGHT,WM_APP+44,Joypad[index].Right,0);
	SendDlgItemMessage(hDlg,IDC_A,WM_APP+44,Joypad[index].A,0);
	SendDlgItemMessage(hDlg,IDC_B,WM_APP+44,Joypad[index].B,0);
	SendDlgItemMessage(hDlg,IDC_X,WM_APP+44,Joypad[index].X,0);
	SendDlgItemMessage(hDlg,IDC_Y,WM_APP+44,Joypad[index].Y,0);
	SendDlgItemMessage(hDlg,IDC_L,WM_APP+44,Joypad[index].L,0);
	SendDlgItemMessage(hDlg,IDC_R,WM_APP+44,Joypad[index].R,0);
	SendDlgItemMessage(hDlg,IDC_START,WM_APP+44,Joypad[index].Start,0);
	SendDlgItemMessage(hDlg,IDC_SELECT,WM_APP+44,Joypad[index].Select,0);
	if(index < 5)
	{
		SendDlgItemMessage(hDlg,IDC_UPLEFT,WM_APP+44,Joypad[index].Left_Up,0);
		SendDlgItemMessage(hDlg,IDC_UPRIGHT,WM_APP+44,Joypad[index].Right_Up,0);
		SendDlgItemMessage(hDlg,IDC_DWNLEFT,WM_APP+44,Joypad[index].Left_Down,0);
		SendDlgItemMessage(hDlg,IDC_DWNRIGHT,WM_APP+44,Joypad[index].Right_Down,0);
	}
}
...
static void set_hotkeyinfo(HWND hDlg)
{
	...
	while (!IsLastCustomKey(key) && i < NUM_HOTKEY_CONTROLS) {
		if (page == key->page) {
			SendDlgItemMessage(hDlg, IDC_HOTKEY_Table[i], WM_APP+44, key->key, key->modifiers);
	...
	for (; i < NUM_HOTKEY_CONTROLS; i++) {
		SendDlgItemMessage(hDlg, IDC_HOTKEY_Table[i], WM_APP+44, 0, 0);
...
///			SendDlgItemMessage(hDlg,IDC_READONLY,BM_SETCHECK,BST_UNCHECKED,0);
		...
		{
			SendDlgItemMessage(hDlg,IDC_WIP1,BM_SETCHECK,              (m.SyncFlags & MOVIE_SYNC_WIP1TIMING)!=0 ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);
		...
		{
			// guess that WIP1 timing should be on (no way to tell from the movie file)
			SendDlgItemMessage(hDlg,IDC_WIP1,BM_SETCHECK, (WPARAM)BST_CHECKED, 0);
//			SendDlgItemMessage(hDlg,IDC_WIP1,BM_SETCHECK, Settings.UseWIPAPUTiming ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);

			// old movies didn't have left+right so disable it on playback by default
			SendDlgItemMessage(hDlg,IDC_ALLOWLEFTRIGHT,BM_SETCHECK, (WPARAM)BST_UNCHECKED, 0);
//			SendDlgItemMessage(hDlg,IDC_ALLOWLEFTRIGHT,BM_SETCHECK, Settings.UpAndDown ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);

			// guess that VEHR was on (no way to tell from the movie file)
			SendDlgItemMessage(hDlg,IDC_ENVX,BM_SETCHECK, (WPARAM)BST_CHECKED, 0);
//			SendDlgItemMessage(hDlg,IDC_ENVX,BM_SETCHECK, Settings.SoundEnvelopeHeightReading ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);
			...
			// old movies didn't have fake mute so disable it on playback by default
			SendDlgItemMessage(hDlg,IDC_FMUT,BM_SETCHECK, (WPARAM)BST_UNCHECKED, 0);
//			SendDlgItemMessage(hDlg,IDC_FMUT,BM_SETCHECK, Settings.FakeMuteFix ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);

			// guess that sync samples was on (no way to tell from the movie file)
			...
			case IDOK:
				{
					if(BST_CHECKED==SendDlgItemMessage(hDlg, IDC_SEEK_TO_FRAME, BM_GETCHECK,0,0))
					{
						...
						{
							Settings.HighSpeedSeek = (frame_dest+1);
							Settings.Paused = false;
							Settings.FrameAdvance = false;
							GUI.FrameAdvanceJustPressed = 0;
						}
	...
	case WM_INITDIALOG:
		{
			...
			SendDlgItemMessage(hDlg,IDC_WIP1,BM_SETCHECK, (WPARAM)BST_CHECKED, 0); // I realized that this should *always* be on when recording, because whenever it makes any difference to have it off, it prevents desyncs. Let the user turn it off each time if they really want it off when recording.
//			SendDlgItemMessage(hDlg,IDC_WIP1,BM_SETCHECK, Settings.UseWIPAPUTiming ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);
			SendDlgItemMessage(hDlg,IDC_ALLOWLEFTRIGHT,BM_SETCHECK, Settings.UpAndDown ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);
//			SendDlgItemMessage(hDlg,IDC_ENVX,BM_SETCHECK, Settings.SoundEnvelopeHeightReading ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);
//			SendDlgItemMessage(hDlg,IDC_FMUT,BM_SETCHECK, Settings.FakeMuteFix ? (WPARAM)BST_CHECKED : (WPARAM)BST_UNCHECKED, 0);
			...
			case IDOK:
				{
					...
					Settings.UseWIPAPUTiming = IsDlgButtonChecked(hDlg, IDC_WIP1);
					Settings.UpAndDown = IsDlgButtonChecked(hDlg, IDC_ALLOWLEFTRIGHT);
...
void WinDisplayChar(screenPtrType *s, uint8 c) {
    ...
    uint32 RealPPL = GFX.Pitch2/2;
	if(!display_paramsinited) display_ppl = Settings.OpenGLEnable ? IPPU.RenderedScreenWidth : RealPPL;
	if(display_hfontaccessscale == 1 && display_vfontaccessscale == 1) {
		if (hiRes && (display_screen == (uint16*)GFX.Screen || GUI.ScaleHiRes == FILTER_NONE)) {
...
//				    * (Settings.SixteenBit ? 2 : 1)
	...
	for(int i = 0 ; i < len ; i++)
	{
		...
		Screen += /*Settings.SixteenBit ? (display_fontwidth-display_hfontaccessscale)*sizeof(uint16) :*/ (display_fontwidth-display_hfontaccessscale) * ((hiRes && (display_screen == (uint16*)GFX.Screen || GUI.ScaleHiRes == FILTER_NONE)) ? 2 : 1);
		char_count++;
	}
